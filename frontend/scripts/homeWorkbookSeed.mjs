...
const workbookCollections = sampleCatalogCollections.filter((collection) => collection.segmentSlug);
const getCollectionProducts = (key) =>
  sampleCatalogCollections.find((collection) => collection.key === key)?.products ?? [];

const archiveProductMix = [
  ...getCollectionProducts("men").slice(0, 3),
  ...getCollectionProducts("women").slice(0, 3),
  ...getCollectionProducts("footwear").slice(0, 3),
  ...getCollectionProducts("accessories").slice(0, 3),
];

function buildArchiveProducts() {
  return archiveProductMix.map((product, index) => ({
    segment_slug: "all-archive",
    position: index + 1,
    ...
    name: product.name,
    price: product.price,
    size_tag: product.material,
    fit_note: `${product.fitNote} Curated into the All Archive rail for a fuller workbook seed.`,
    image_url: product.imageUrl,
    href: "/products",
  }));
}

function buildSegmentProducts() {
  return workbookCollections.flatMap((collection) =>
    collection.products.map((product, index) => ({
      segment_slug: collection.segmentSlug,
      position: index + 1,
      product_id: product.productId,
      eyebrow: product.segmentEyebrow,
      ...
      image_url: product.imageUrl,
      href: collection.workbookHref,
    }))
  );
}

export const products = [...buildArchiveProducts(), ...buildSegmentProducts()];
...
export const categoryPageProducts = sampleCatalogCollections.flatMap((collection) =>
  collection.products.map((product, index) => ({
    page_slug: collection.pageSlug,
    position: index + 1,
    product_id: product.productId,
    badge: product.badge,
    name: product.name,
    ...
    href: collection.workbookHref,
    filter_tags: product.filterTags.join("|"),
  }))
);
