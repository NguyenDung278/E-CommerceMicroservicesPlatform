...
function matchesStorefrontCategoryToken(normalizedCategory: string, token: string) {
...
export function getStorefrontArchiveCategoryLabel(categoryName: string) {
  ...
  return (
    storefrontArchiveCategoryMappings.find(({ tokens }) =>
      tokens.some((token) => matchesStorefrontCategoryToken(normalizedCategory, token))
    )?.label ?? ""
  );
}
...
export function isStorefrontAutoAddCategory(categoryName: string) {
  ...
  return storefrontAutoAddCategoryTokens.some((token) =>
...
function isSameNavigationItem(left: StorefrontNavigationItem, right: StorefrontNavigationItem) {
  if (left.to === right.to) {
    return true;
  }
...
export function mergeWithStorefrontNavigation(items: StorefrontNavigationItem[]) {
