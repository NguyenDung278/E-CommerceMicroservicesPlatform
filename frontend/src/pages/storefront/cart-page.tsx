...
export function CartPage() {
    ...
    void Promise.allSettled(
      cart.items.map((item) =>
        api
          ...
          .then((response) => [item.product_id, response.data] as const)
      )
    ).then((results) => {
      if (!active) {
        return;
  ...
  async function handlePreviewCoupon() {
    ...
    try {
      ...
      setCouponFeedback(
        `Voucher ${response.data.coupon_code ?? normalizedCouponCode.toUpperCase()} đã được áp dụng.`
      );
    } catch (reason) {
      setCouponPreview(null);
    ...
    <div className="page-stack cart-editorial-page">
      ...
      {!isAuthenticated && cart.items.length > 0 ? (
        <div className="feedback feedback-info">
          Đây là giỏ hàng dành cho khách vãng lai trên thiết bị hiện tại. Khi bạn đăng nhập, hệ
          thống sẽ tự động merge các món này vào tài khoản.
        </div>
      ) : null}
                ...
                <article className="cart-editorial-item" key={item.product_id}>
                  <div className="cart-editorial-media">
                    {imageUrl ? (
                      ...
                      <div className="cart-editorial-fallback">
                      ...
                      <div className="cart-editorial-meta-block">
                        ...
                        <div className="cart-editorial-quantity">
                          <button
                            type="button"
                            onClick={() =>
                              void handleQuantityChange(item.product_id, item.quantity - 1)
                            }
                          >
                            -
                          </button>
              ...
              {couponPreview?.discount_amount ? (
                <>
                  <div className="summary-row coupon-summary-discount">
                    <span>
                      Giảm giá {couponPreview.coupon_code ? `(${couponPreview.coupon_code})` : ""}
                    </span>
