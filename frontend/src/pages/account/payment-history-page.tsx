...
import "@/styles/pages/account/payment-history-page.css";
...
export function PaymentHistoryPage() {
  ...
  const pendingCount = paymentEntries.filter((entry) =>
    entry.payment.status.toLowerCase().includes("pending")
  ).length;
  const failedCount = paymentEntries.filter((entry) =>
    entry.payment.status.toLowerCase().includes("fail")
  ).length;
      ...
      <div className="payments-route">
        <section className="payments-route-hero">
          <div className="payments-route-hero-copy">
            ...
            <p>
              Securely manage your saved payment options and review your recent billing history
              within the ND Shop account area.
            </p>
          </div>
        ...
        <section className="payments-route-overview">
          <div className="payments-route-methods">
            {paymentMethodHighlights.length === 0 ? (
              <div className="payments-route-empty">
                ...
                <p>
                  Once checkout succeeds, the payment methods you use most will be summarized here.
                </p>
              </div>
            ) : (
                ...
                <article className="payments-route-card" key={item.key}>
                  <div className="payments-route-card-head">
                    <div className="payments-route-card-ident">
                      <span className="payments-route-card-badge">
                        {buildTileLabel(item.title)}
                      </span>
                      <div>
                        <h3>{item.title}</h3>
                        <p>{item.subtitle}</p>
          ...
          <div className="payments-route-rail">
            <article className="payments-route-note">
              ...
              <p>
                Your payment data is encrypted and handled by gateway providers. We only surface the
                transaction trail and verification state.
              </p>
            </article>
            ...
            <article className="payments-route-note payments-route-note-accent">
              ...
              <strong>
                {paymentEntries[0]
                  ? formatDateTime(paymentEntries[0].payment.created_at)
                  : "No records yet"}
              </strong>
            </article>
          </div>
                ...
                <tbody>
                  {paymentEntries.map(({ payment, orderCreatedAt }) => (
                    <tr key={payment.id}>
                      ...
                      <td>
                        <span
                          className={`payments-route-status payments-route-status-${getPaymentStatusTone(payment.status)}`}
                        >
