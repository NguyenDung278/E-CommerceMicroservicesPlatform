...
import { authApi, type OAuthProvider } from "@/services/api/modules/auth-api";
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function updateProfile(input: UpdateProfileInput) {
    ...
    const response = await withFreshToken((activeToken) =>
      authApi.updateProfile(activeToken, input)
    );
    startTransition(() => {
      setUser(response.data);
      setError("");
  ...
  const getPhoneVerificationStatus = useCallback(async () => {
    setError("");
    const response = await withFreshToken((activeToken) =>
      authApi.getPhoneVerificationStatus(activeToken)
    );
    return response.data;
  }, [withFreshToken]);
