...
export function normalizeStorefrontCategoryList(value: unknown): StorefrontCategory[] {
  return Array.isArray(value) ? value.map((item) => normalizeStorefrontCategory(item)) : [];
}
...
export function normalizeStorefrontFeaturedProduct(value: unknown): StorefrontFeaturedProduct {
  ...
  return {
    id: normalizeString(product.id),
    product_external_id: normalizeString(product.product_external_id),
    category_slug: normalizeString(product.category_slug),
    position: normalizeNumber(product.position),
    product: normalizeStorefrontProduct(product.product),
  };
}
...
export function normalizeStorefrontCategoryPageData(value: unknown): StorefrontCategoryPageData {
  ...
  return {
    ...
    sections: Array.isArray(pageData.sections)
      ? pageData.sections.map((item) => normalizeStorefrontEditorialSection(item))
      : [],
    featured_products: Array.isArray(pageData.featured_products)
      ? pageData.featured_products.map((item) => normalizeStorefrontFeaturedProduct(item))
      : [],
  };
}

export function normalizeStorefrontHomeData(value: unknown): StorefrontHomeData {
  const homeData = isRecord(value) ? value : {};

  return {
    categories: Array.isArray(homeData.categories)
      ? homeData.categories.map((item) => normalizeStorefrontCategory(item))
      : [],
    category_pages: Array.isArray(homeData.category_pages)
      ? homeData.category_pages.map((item) => normalizeStorefrontCategoryPageData(item))
      : [],
  };
}
 ...
 */
export function normalizeProductPopularityList(value: unknown): ProductPopularity[] {
  return Array.isArray(value) ? value.map((item) => normalizeProductPopularity(item)) : [];
}
...
export function normalizePhoneVerificationChallenge(
  value: unknown
...
function normalizeAdminOrderStatusBreakdown(value: unknown): AdminOrderStatusBreakdown {
  const item = isRecord(value) ? value : {};
