...
export const storefrontApi = {
  ...
  getCategoryPage(identifier: string): Promise<ApiEnvelope<StorefrontCategoryPageData>> {
    return request<unknown>(`/api/v1/storefront/categories/${encodeURIComponent(identifier)}`).then(
      (response) => ({
        ...response,
