...
export const authApi = {
  ...
  getPhoneVerificationStatus(
    token: string
  ): Promise<ApiEnvelope<PhoneVerificationChallenge | null>> {
    return request<unknown>("/api/v1/users/profile/phone-verification", { token }).then(
      (response) => ({
        ...response,
