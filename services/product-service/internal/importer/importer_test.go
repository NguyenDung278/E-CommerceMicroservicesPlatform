...
func TestLoadWorkbookParsesSampleWorkbook(t *testing.T) {
	...
	if len(workbook.Categories) < 2 {
		t.Fatalf("expected sample workbook to include at least 2 categories, got %d", len(workbook.Categories))
	}
	if len(workbook.Products) < 3 {
		t.Fatalf("expected sample workbook to include at least 3 products, got %d", len(workbook.Products))
	}
	if len(workbook.Variants) < 3 {
		t.Fatalf("expected sample workbook to include at least 3 variants, got %d", len(workbook.Variants))
	}
	if len(workbook.EditorialSections) < 3 {
		t.Fatalf("expected sample workbook to include at least 3 editorial sections, got %d", len(workbook.EditorialSections))
	}
