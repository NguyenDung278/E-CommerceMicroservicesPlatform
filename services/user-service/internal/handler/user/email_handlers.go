...
func handleEmailOTPError(c echo.Context, err error) error {
	var emailVerificationErr *account.EmailVerificationError
	...
	case errors.Is(err, account.ErrEmailVerificationLocked):
	...
	case errors.Is(err, account.ErrEmailVerificationResendTooSoon):
