...
func NewEmailVerificationRepository(db *sql.DB) EmailVerificationRepository {
	return newEmailVerificationRepositoryWithExecutor(db)
}

func newEmailVerificationRepositoryWithExecutor(executor sqlExecutor) EmailVerificationRepository {
	return &postgresEmailVerificationRepository{executor: executor}
}

func (r *postgresEmailVerificationRepository) Create(ctx context.Context, challenge *model.EmailVerificationChallenge) error {
	query := `
...
func (r *postgresEmailVerificationRepository) GetByID(ctx context.Context, id string) (*model.EmailVerificationChallenge, error) {
	query := `
...
func (r *postgresEmailVerificationRepository) GetLatestActiveByUserID(ctx context.Context, userID, purpose string) (*model.EmailVerificationChallenge, error) {
	query := `
		SELECT id, user_id, purpose, email, otp_hash, expires_at,
		       resend_available_at, last_sent_at, attempt_count, max_attempts,
		       status, verified_at, consumed_at, created_at, updated_at
...
func (r *postgresEmailVerificationRepository) Update(ctx context.Context, challenge *model.EmailVerificationChallenge) error {
	query := `
