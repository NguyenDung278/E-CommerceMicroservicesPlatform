...
type PhoneSignupRepository struct {
	executor common.SQLExecutor
}

func NewPhoneSignup(db *sql.DB) *PhoneSignupRepository {
	return NewPhoneSignupWithExecutor(db)
}

func NewPhoneSignupWithExecutor(executor common.SQLExecutor) *PhoneSignupRepository {
	return &PhoneSignupRepository{executor: executor}
}

func (r *PhoneSignupRepository) Create(ctx context.Context, challenge *model.PhoneSignupChallenge) error {
	query := `
...
func (r *PhoneSignupRepository) GetByID(ctx context.Context, id string) (*model.PhoneSignupChallenge, error) {
	query := `
...
func (r *PhoneSignupRepository) GetLatestActiveByPhone(ctx context.Context, phone string) (*model.PhoneSignupChallenge, error) {
	query := `
		SELECT id, phone, password_hash, first_name, last_name, otp_hash, expires_at,
		       resend_available_at, last_sent_at, attempt_count, max_attempts,
		       status, telegram_chat_id, verified_at, consumed_at, created_at, updated_at
...
func (r *PhoneSignupRepository) Update(ctx context.Context, challenge *model.PhoneSignupChallenge) error {
	query := `
...
func (r *PhoneSignupRepository) DeleteExpired(ctx context.Context) error {
