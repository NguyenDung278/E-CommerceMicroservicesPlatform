...
type postgresEmailSignupRepository struct {
	executor sqlExecutor
}

func NewEmailSignupRepository(db *sql.DB) EmailSignupRepository {
	return newEmailSignupRepositoryWithExecutor(db)
}

func newEmailSignupRepositoryWithExecutor(executor sqlExecutor) EmailSignupRepository {
	return &postgresEmailSignupRepository{executor: executor}
}

func (r *postgresEmailSignupRepository) Create(ctx context.Context, challenge *model.EmailSignupChallenge) error {
	query := `
...
func (r *postgresEmailSignupRepository) GetByID(ctx context.Context, id string) (*model.EmailSignupChallenge, error) {
	query := `
...
func (r *postgresEmailSignupRepository) GetLatestActiveByEmail(ctx context.Context, email string) (*model.EmailSignupChallenge, error) {
	query := `
		SELECT id, email, password_hash, first_name, last_name, otp_hash, expires_at,
		       resend_available_at, last_sent_at, attempt_count, max_attempts,
		       status, verified_at, consumed_at, created_at, updated_at
...
func (r *postgresEmailSignupRepository) Update(ctx context.Context, challenge *model.EmailSignupChallenge) error {
	query := `
...
func (r *postgresEmailSignupRepository) DeleteExpired(ctx context.Context) error {
