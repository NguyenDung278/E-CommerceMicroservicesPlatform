...
_(Lưu ý: Bạn cần dán source code tương ứng vào cùng prompt dưới đây)_
...
> "Dưới đây là nội dung của một page trong `frontend/src/pages`. Hãy giải thích luồng dữ liệu của page này theo thứ tự: route -> component state -> hook/provider -> API module -> type/normalizer -> output UI. Chỉ rõ side effect, edge case, trade-off và điểm nào nên refactor nếu app tiếp tục lớn hơn."
