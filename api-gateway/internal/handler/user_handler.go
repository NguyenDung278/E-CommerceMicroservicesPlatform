...
func (h *UserHandler) RegisterRoutes(e *echo.Echo, jwtSecret string) {
	...
	users.GET("/verify-email/status", h.forward)
	users.POST("/verify-email/send-otp", h.forward)
	users.POST("/verify-email/verify-otp", h.forward)
	users.POST("/verify-email/resend-otp", h.forward)
